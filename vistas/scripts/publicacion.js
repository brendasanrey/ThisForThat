...
function init(){
	...
	/*
	$.post("../ajax/publicacion.php?op=selectCategoria", function(r){
	            $("#idcategoria").html(r);
...
function listar(){
	tabla = $('#tbllistado').dataTable({
		...
		"ajax":{
			url: '../ajax/publicacion.php?op=listar',
			type: "get",
			dataType: "json",
			error: function(e){
...
function guardar(e){
	...
	console.log("Entro");	
	$.ajax({
		url: "../ajax/publicacion.php?op=guardar",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
		success: function(datos){
			if(datos){
				swal(
				  'Registrada',
				  'La publicación se registro correctamente',
				  'success'
				)
			}else{
				swal(
				  'Error',
				  'La publicación no se pudo registrar, intente de nuevo.',
				  'error'
				)
			}
			mostrarform(false);
